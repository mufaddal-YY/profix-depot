  ...
  const automaticBlockquoteOnSpace = behavior_core.defineBehavior({
    ...
    guard: ({
      ...
      const previousInlineObject = selector_isAtTheStartOfBlock.getPreviousInlineObject({
        context
      }), blockOffset = util_sliceBlocks.spanSelectionPointToBlockOffset({
        value: context.value,
        selectionPoint: {
          path: [{
      ...
      if (previousInlineObject || !blockOffset)
        return !1;
      const blockText = util_sliceBlocks.getTextBlockText(focusTextBlock.node), caretAtTheEndOfQuote = blockOffset.offset === 1, looksLikeMarkdownQuote = /^>/.test(blockText), blockquoteStyle = config.blockquoteStyle?.(context);
    ...
    guard: ({
      ...
      const previousInlineObject = selector_isAtTheStartOfBlock.getPreviousInlineObject({
        context
      }), blockText = util_sliceBlocks.getTextBlockText(focusTextBlock.node), markdownHeadingSearch = /^#+/.exec(blockText), level = markdownHeadingSearch ? markdownHeadingSearch[0].length : void 0, caretAtTheEndOfHeading = blockOffset.offset === level;
    ...
    guard: ({
      ...
      if (previousInlineObject || !blockOffset)
        return !1;
      const blockText = util_sliceBlocks.getTextBlockText(focusTextBlock.node), defaultStyle = config.defaultStyle?.(context), looksLikeUnorderedList = /^(-|\*)/.test(blockText), unorderedListStyle = config.unorderedListStyle?.(context), caretAtTheEndOfUnorderedList = blockOffset.offset === 1;
