...
const DocumentReporter = (props) => {
  const {
    documentIds
  } = props, [uniqueIds, setUniqueIds] = react.useState([]);
  react.useEffect(() => {
    setUniqueIds((prev) => {
  ...
  $[1] !== comlink2 ? (t2 = () => {
    const unsubs = [comlink2?.on("presentation/focus", (data) => {
      ...
      setOverlayEnabled(_temp$1);
    })].filter(Boolean);
    return () => unsubs.forEach(_temp2$1);
  ...
  $[6] === Symbol.for("react.memo_cache_sentinel") ? (t5 = () => {
    const handleClick = _temp3, handleKeyUp = (e) => {
      isAltKey(e) && setOverlayEnabled(_temp4);
    }, handleKeydown = (e_0) => {
      isAltKey(e_0) && setOverlayEnabled(_temp5), isHotkey(["mod", "\\"], e_0) && setOverlayEnabled(_temp6);
    };
  ...
  let t9, t10;
  $[11] !== elements ? (t10 = elements.flatMap(_temp7), $[11] = elements, $[12] = t10) : t10 = $[12], t9 = t10;
  ...
  bb0: {
    ...
    let t142;
    $[15] !== componentResolver || $[16] !== dragMinimapTransition || $[17] !== dragShowMinimap || $[18] !== elements || $[19] !== inFrame || $[20] !== isDragging || $[21] !== optimisticActorReady || $[22] !== wasMaybeCollapsed ? (t142 = elements.filter(_temp8).map((t152) => {
      const {
        id,
        element: element_0,
        focused,
...
function _temp4(enabled_0) {
  return !enabled_0;
}
function _temp5(enabled_1) {
  return !enabled_1;
}
function _temp6(enabled_2) {
  return !enabled_2;
}
function _temp7(element) {
